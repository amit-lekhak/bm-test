...
type LoaderData = {
  data: { name: string; avatar: string; time: string };
};
...
export default function MeetingDetails() {
  ...
  const timeList = useMemo(() => {
    ...
    let newDate = new Date(startTime + parseInt(data.time, 10) * 60 * 1000);
    while (newDate < new Date(endTime)) {
      date.push(newDate);
      newDate = new Date(
        newDate.getTime() + parseInt(data.time, 10) * 60 * 1000
      );
    }
        ...
        <section className="max-w-[33%] ">
          <div className="flex flex-col p-5 ">
            ...
            <h1 className="mb-6 break-words text-2xl font-semibold text-gray-900">
              <p className="ml-2">{data.time} Min Meeting</p>
            </h1>
          ...
          <section className="h-full w-full max-w-[33%]">
            <div className="flex flex-col  p-4 text-center ">
              ...
              <div className="flex flex-col gap-2 h-[calc(414px-16px)] overflow-y-auto ">
                {timeList.map((dateList, i) => {
                  let hour;
                  let minute;
                  let ampm: "am" | "pm";

                  const tempHour = new Date(dateList).getHours();
                  const tempMinute = new Date(dateList).getMinutes();
